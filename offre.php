...
try {
    $stmt = $pdo->prepare("
        SELECT 
            ...
            oe.id_recruteur,
            r.nom_entreprise,
            s.description_secteur as description_entreprise,
            s.nom_secteur,
            d.nom_domaine
...
// Récupérer les offres similaires (même secteur ou domaine)
try {
    $stmt_similar = $pdo->prepare("
        ...
        FROM offres_emploi oe
        LEFT JOIN recruteurs r ON oe.id_recruteur = r.id_recruteur
        WHERE (oe.id_secteur = (SELECT id_secteur FROM offres_emploi WHERE id_offre = :id_offre)
            OR oe.id_domaine = (SELECT id_domaine FROM offres_emploi WHERE id_offre = :id_offre2))
            AND oe.id_offre != :id_offre3
    ...
    $offres_similaires = $stmt_similar->fetchAll();
} catch (PDOException $e) {
    $offres_similaires = [];
}
...
<head>
    ...
    <title><?= htmlspecialchars($offre['titre_emploi']) ?> - NextCareer</title>
...
<nav class="bg-white shadow-sm sticky top-0 z-50 border-b border-gray-200">
                ...
                <?php else: ?>
                    ...
                    <div class="hidden sm:flex items-center space-x-3 border-r border-gray-300 pr-4">
                        <div class="w-10 h-10 <?= $bg_color ?> rounded-full flex items-center justify-center">
                            <span class="text-lg font-bold <?= $text_color ?>"><?= $initiales ?></span>
                        </div>
                        <div class="text-sm">
    ...
    <div class="bg-white border-b">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <a href="offres.php" class="inline-flex items-center gap-2 text-gray-600 hover:text-indigo-600 transition font-medium">
                <span>←</span>
                Retour aux offres
            </a>
        </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 relative overflow-hidden">
                    ...
                    <div class="flex items-start justify-between mb-6">
                        <div class="flex-1 pr-4">
                            ...
                            <p class="text-xl text-gray-600 font-semibold mb-4">
                            ...
                            <div class="flex flex-wrap items-center gap-3">
                                ...
                                <span class="bg-blue-50 text-blue-700 px-3 py-1.5 rounded-full font-semibold text-sm border border-blue-200">
                                    <?= htmlspecialchars($offre['statut_recherche']) ?>
                                </span>
                                <?php endif; ?>
                                
                                <?php if (!empty($offre['nom_secteur'])): ?>
                                <span class="bg-purple-50 text-purple-700 px-3 py-1.5 rounded-full font-semibold text-sm border border-purple-200">
                        ...
                        <button class="p-3 hover:bg-gray-50 rounded-lg transition" title="Ajouter aux favoris">
                    ...
                    <!-- Boutons d'action -->
                    <?php if (isset($_SESSION['user_type']) && $_SESSION['user_type'] === 'candidat'): ?>
                        <?php if ($has_applied): ?>
                            <div class="bg-green-50 border-2 border-green-200 text-green-700 px-6 py-4 rounded-xl flex items-center gap-3">
                                
                                <span class="font-semibold">Vous avez déjà postulé à cette offre</span>
                            </div>
                        <?php else: ?>
                            <a href="pages/candidat/postuler.php?id=<?= $offre['id_offre'] ?>" class="block w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-center py-4 rounded-xl hover:shadow-lg transition font-bold text-lg">
                                Postuler maintenant →
                            </a>
                        <?php endif; ?>
                    <?php elseif (!isset($_SESSION['user_id'])): ?>
                        <div class="bg-yellow-50 border-2 border-yellow-200 rounded-xl p-6">
                            <p class="text-yellow-800 font-semibold mb-4 flex items-center gap-2">
                                <span>⚠️</span>
                                Vous devez être connecté pour postuler
                            </p>
                            <div class="flex gap-3">
                                <a href="auth/login.php" class="flex-1 bg-indigo-600 text-white text-center py-3 rounded-lg hover:bg-indigo-700 transition font-semibold">
                                    Se connecter
                                </a>
                                <a href="auth/register.php?type=candidat" class="flex-1 bg-white text-indigo-600 border-2 border-indigo-600 text-center py-3 rounded-lg hover:bg-indigo-50 transition font-semibold">
                                    S'inscrire
                                </a>
                            </div>
                        </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                    ...
                    <div class="prose max-w-none text-gray-700 leading-relaxed">
                        <?= nl2br(htmlspecialchars($offre['description_offre'])) ?>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                    ...
                    <div class="flex flex-wrap gap-3">
                        ...
                        ?>
                        <span class="bg-indigo-50 text-indigo-700 px-4 py-2.5 rounded-xl font-semibold border border-indigo-200">
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-24">
                    ...
                    <div class="space-y-4 text-sm">
                        <?php if (!empty($offre['nom_secteur'])): ?>
                        <div>
                            <p class="text-gray-500 text-xs mb-1">Secteur</p>
                            <p class="text-gray-800 font-semibold"><?= htmlspecialchars($offre['nom_secteur']) ?></p>
                        </div>
                        <?php endif; ?>

                        <?php if (!empty($offre['secteur_activite'])): ?>
                        <div>
                            <p class="text-gray-500 text-xs mb-1">Activité</p>
                            <p class="text-gray-800"><?= htmlspecialchars($offre['secteur_activite']) ?></p>
                        </div>
                        <?php endif; ?>
                        ...
                        <div>
                            ...
                            <p class="text-gray-700 text-sm leading-relaxed"><?= htmlspecialchars($offre['description_entreprise']) ?></p>
                        </div>
                        <?php endif; ?>
                    </div>

                    <button class="w-full mt-6 bg-gray-100 text-gray-700 py-3 rounded-xl hover:bg-gray-200 transition font-semibold flex items-center justify-center gap-2">
                        <span>🔗</span>
                ...
                <!-- Offres similaires -->
                <?php if (!empty($offres_similaires)): ?>
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-3">
                        <?php foreach ($offres_similaires as $similar): ?>
                        <a href="offre.php?id=<?= $similar['id_offre'] ?>" class="block p-4 rounded-xl hover:bg-gray-50 transition border border-gray-100 group">
                            <h4 class="font-bold text-gray-800 mb-1 group-hover:text-indigo-600 transition">
                                <?= htmlspecialchars($similar['titre_emploi']) ?>
                            </h4>
                            <p class="text-sm text-gray-600 mb-2"><?= htmlspecialchars($similar['nom_entreprise']) ?></p>
                            <p class="text-xs text-gray-500 flex items-center gap-1">
                                <span>📍</span>
                                <?= htmlspecialchars($similar['localisation']) ?>
                            </p>
                        </a>
                        <?php endforeach; ?>
                    </div>
